...
check('every tool has a category', TOOLS.every((t) => TOOL_CATEGORIES.includes(t.category)))
check(
  'reconciled real tools are present',
  ['task', 'skill', 'lsp', 'browser_close', 'loop_create', 'loop_remove', 'change_session_metadata'].every(
    (id) => Boolean(getTool(id))
  )
)
check(
  'removed aspirational tools are gone',
  ['apply_patch', 'todowrite', 'question', 'list_sessions', 'check_session'].every((id) => !getTool(id))
)
...
check(
  ...
  (() => {
    ...
    return r.length === 1 && r[0].role === 'assistant' && r[0].content === 'the answer' && !r[0].toolCalls
  })()
)
...
check(
  ...
  (() => {
    const r = reconstructAssistant([
      ...
      { type: 'tool', tool: 'read', state: 'done', callId: 'a', input: { path: 'x.ts' }, output: 'AAA' },
      { type: 'tool', tool: 'grep', state: 'done', callId: 'b', input: { pattern: 'foo' }, output: 'BBB' },
      { type: 'text', text: 'done' }
    ])
    const [a0, t0, t1, a1] = r
    return (
...
check(
  ...
  (() => {
    ...
    const callIds = r.filter((m) => m.role === 'assistant').flatMap((m) => m.toolCalls?.map((c) => c.id) ?? [])
...
check(
  ...
  (() => {
    const r = reconstructAssistant([{ type: 'tool', tool: 'read', state: 'done', callId: 'a', input: {} }])
...
check(
  ...
  (() => {
    ...
    const r = reconstructAssistant([{ type: 'tool', tool: 'read', state: 'done', callId: 'a', input: {}, output: big }])
    const toolMsg = r.find((m) => m.role === 'tool')
    const content = toolMsg?.content ?? ''
    return content.length <= REPLAY_OUTPUT_CAP + 200 && content.startsWith('z') && content.includes('truncated')
  })()
)
...
check(
  ...
  (() => {
    const structured: ChatMessage[] = [
      ...
      { role: 'assistant', content: 'checking', toolCalls: [{ id: 'a', name: 'read', arguments: '{}' }] },
...
check('normalizeFetchUrl upgrades http→https', normalizeFetchUrl('http://example.com/x') === 'https://example.com/x')
check('normalizeFetchUrl keeps https', normalizeFetchUrl('https://example.com/') === 'https://example.com/')
check(
...
check('acceptHeader markdown prefers markdown', acceptHeader('markdown').startsWith('text/markdown'))
check('mimeFromContentType strips charset', mimeFromContentType('text/html; charset=utf-8') === 'text/html')
...
check('isTextualMime true for json', isTextualMime('application/json') && isTextualMime('text/plain'))
check('isTextualMime false for pdf', !isTextualMime('application/pdf'))
check('decodeEntities named + numeric', decodeEntities('a &amp; b &lt;c&gt; &#65; &#x42;') === 'a & b <c> A B')
check(
  'htmlToText strips tags + script/style',
  (() => {
    const html = '<html><head><style>.x{color:red}</style></head><body><h1>Title</h1><script>evil()</script><p>Hello <b>world</b>.</p></body></html>'
    const t = htmlToText(html)
    return t.includes('Title') && t.includes('Hello world.') && !t.includes('evil') && !t.includes('color:red') && !t.includes('<')
  })()
)
check(
...
check('clampResults default when invalid', clampResults('abc') === WEBSEARCH_DEFAULT_RESULTS && clampResults(0) === WEBSEARCH_DEFAULT_RESULTS)
...
check('parseExaResponse returns undefined on empty/garbage', parseExaResponse('not json') === undefined)
...
check('previewText returns short text unchanged', previewText('just a line', { maxLines: 40, maxChars: 400 }) === 'just a line')
const bigPreview = previewText('L'.repeat(5_000) + '\nTAILMARK', { maxLines: 40, maxChars: 400, marker: '[[cut]]' })
...
const convo = [
  ...
  { role: 'assistant', content: '', tool_calls: [{ id: 'a', type: 'function', function: { name: 'grep', arguments: '{}' } }] },
...
check('pruneToolMessages preserves length + order', prunedConvo.length === convo.length && prunedConvo[0] === convo[0])
check('pruneToolMessages shrinks the OLD tool output', (prunedConvo[2].content as string).length < bigOut.length)
check('pruneToolMessages keeps the RECENT tool output intact', prunedConvo[prunedConvo.length - 1].content === bigOut)
check(
...
const replayTurn: Message = {
  ...
  content: '',
  parts: [
    { type: 'tool', tool: 'grep', callId: 'r1', input: {}, output: 'HEAD'.repeat(3_000) + 'UNIQUETAIL', state: 'done' }
  ] as MessagePart[],
  createdAt: 1
} as Message
...
check('reconstruct preview stays within the replay cap window', (replayedTool?.content ?? '').length <= REPLAY_OUTPUT_CAP + 200)
...
check('partitionToolCalls preserves task order', partitioned.tasks.map((c) => c.id).join() === 'a,c')
check(
...
check('parseTaskInput defaults subagent to general', parseTaskInput('{}').subagentType === 'general')
check('parseTaskInput default description', parseTaskInput('{}').description === 'subtask')
check('parseTaskInput background=true (bool)', parseTaskInput(JSON.stringify({ background: true })).background === true)
check('parseTaskInput background="true" (string)', parseTaskInput(JSON.stringify({ background: 'true' })).background === true)
check('parseTaskInput background="1"', parseTaskInput(JSON.stringify({ background: '1' })).background === true)
check(
  'parseTaskInput other background string is false',
  parseTaskInput(JSON.stringify({ background: 'nope' })).background === false
)
check('parseTaskInput task_id passthrough', parseTaskInput(JSON.stringify({ task_id: 'sess_9' })).taskId === 'sess_9')
check('parseTaskInput task_id absent → undefined', parseTaskInput('{}').taskId === undefined)
check('parseTaskInput tolerates malformed JSON', parseTaskInput('{not json').subagentType === 'general')
...
check(
  ...
  renderTaskResult('explore', 'completed', 'x', 'a summary').includes('<summary>a summary</summary>')
)
...
check('encodeRpcMessage writes a Content-Length header', /^Content-Length: \d+\r\n\r\n/.test(framedText))
...
check('RpcDecoder completes a split message', rest.length === 1 && (rest[0] as { id?: number }).id === 7)
...
check('prettyDiagnostic is 1-based with source', prettyDiagnostic(errDiag) === 'ERROR [5:3] Cannot find name x (ts)')

const errBlock = renderDiagnosticsBlock('src/a.ts', [errDiag, warnDiag])
check('renderDiagnosticsBlock wraps in a diagnostics tag', errBlock.startsWith('<diagnostics file="src/a.ts">'))
...
check('renderDiagnosticsBlock clean file → empty string', renderDiagnosticsBlock('x.ts', [warnDiag]) === '')
check(
...
check('renderDiagnosticsBlock sorts by position', renderDiagnosticsBlock('x.ts', [
  { range: { start: { line: 3, character: 0 }, end: { line: 3, character: 1 } }, severity: 1, message: 'later' },
  { range: { start: { line: 1, character: 0 }, end: { line: 1, character: 1 } }, severity: 1, message: 'earlier' }
]).indexOf('earlier') < renderDiagnosticsBlock('x.ts', [
  { range: { start: { line: 3, character: 0 }, end: { line: 3, character: 1 } }, severity: 1, message: 'later' },
  { range: { start: { line: 1, character: 0 }, end: { line: 1, character: 1 } }, severity: 1, message: 'earlier' }
]).indexOf('later'))
...
check('mcp cfg: local from command array', JSON.stringify(normalizeServerConfig({ command: ['node', 'x.js'] })) === JSON.stringify({ type: 'local', command: ['node', 'x.js'] }))
const localStrCmd = normalizeServerConfig({ command: 'node', args: ['x.js'] })
check('mcp cfg: local from command string + args', localStrCmd?.type === 'local' && JSON.stringify((localStrCmd as { command: string[] }).command) === JSON.stringify(['node', 'x.js']))
check('mcp cfg: remote inferred from url', JSON.stringify(normalizeServerConfig({ url: 'https://e.com/mcp' })) === JSON.stringify({ type: 'remote', url: 'https://e.com/mcp' }))
check('mcp cfg: explicit type honored', normalizeServerConfig({ type: 'remote', url: 'https://e.com' })?.type === 'remote')
const localEnv = normalizeServerConfig({ command: ['x'], env: { A: '1' }, timeout: '5000' })
check('mcp cfg: env alias + timeout coercion', localEnv?.type === 'local' && (localEnv as { environment?: Record<string, string>; timeout?: number }).environment?.A === '1' && (localEnv as { timeout?: number }).timeout === 5000)
check('mcp cfg: null/empty/garbage → null', normalizeServerConfig(null) === null && normalizeServerConfig({}) === null && normalizeServerConfig({ command: [] }) === null && normalizeServerConfig({ url: '' }) === null)

// normalizeServerRecords: `{name: config}` map with disabled/enabled honored.
const recs = normalizeServerRecords({ a: { command: ['x'], disabled: true }, b: { url: 'https://e.com' }, bad: {}, c: { command: ['y'], enabled: false } })
check('mcp recs: parses valid entries, skips bad', recs.length === 3 && recs.map((r) => r.id).join(',') === 'a,b,c')
...
check('mcp recs: non-object → []', normalizeServerRecords(null).length === 0 && normalizeServerRecords([]).length === 0)

// qualifyToolName / isMcpToolName: provider-legal, namespaced, collision-resistant.
check('mcp name: qualifies as mcp__server__tool', qualifyToolName('srv', 'tool') === 'mcp__srv__tool')
check('mcp name: sanitizes illegal chars', qualifyToolName('my server', 'do/it') === 'mcp__my_server__do_it')
...
check('mcp name: overlong truncated to <= MAX_TOOL_NAME', longA.length <= MAX_TOOL_NAME && longA.startsWith('mcp__server__'))
check('mcp name: distinct long names stay distinct (hash)', longA !== longB)
check('isMcpToolName: true for namespaced, false otherwise', isMcpToolName('mcp__x__y') && !isMcpToolName('read') && !isMcpToolName('mcpx'))

// mcpToolToSchema: guarantees an object schema; falls back on description.
const sch = mcpToolToSchema('mcp__s__t', 'desc', { type: 'object', properties: { a: { type: 'string' } } })
check('mcp schema: name + description + object params', sch.type === 'function' && sch.function.name === 'mcp__s__t' && sch.function.description === 'desc' && sch.function.parameters.type === 'object' && !!(sch.function.parameters.properties as Record<string, unknown>).a)
check('mcp schema: empty description → fallback names the tool', (mcpToolToSchema('mcp__s__t', '  ', {}).function.description ?? '').includes('mcp__s__t'))
check('mcp schema: non-object inputSchema → {type:object,properties:{}}', JSON.stringify(mcpToolToSchema('mcp__s__t', 'd', 'nope').function.parameters) === JSON.stringify({ type: 'object', properties: {} }))
check('mcp schema: missing properties gets an empty map', JSON.stringify((mcpToolToSchema('mcp__s__t', 'd', { type: 'object' }).function.parameters as { properties: unknown }).properties) === JSON.stringify({}))

// renderMcpContent: text join, image data-url, resource, error mapping.
const rText = renderMcpContent([{ type: 'text', text: 'hello' }, { type: 'text', text: 'world' }], false)
...
check('mcp render: image → data URL + [image] marker', rImg.image === 'data:image/png;base64,AAA' && rImg.output.includes('[image: image/png]'))
check('mcp render: resource with text uses the text', renderMcpContent([{ type: 'resource', resource: { uri: 'file://x', text: 'body' } }], false).output === 'body')
check('mcp render: resource without text → uri pointer', renderMcpContent([{ type: 'resource', resource: { uri: 'file://x' } }], false).output.includes('[resource: file://x]'))
const rErr = renderMcpContent([{ type: 'text', text: 'bad' }], true)
check('mcp render: isError → ok:false', !rErr.ok && rErr.output === 'bad')
check('mcp render: empty content → placeholder', renderMcpContent([], false).output === '(no output)' && renderMcpContent([], false).ok)
check('mcp render: empty error → error placeholder', !renderMcpContent([], true).ok && renderMcpContent([], true).output.includes('error'))
...
check('mcp prompt: lists connected servers + tools + namespacing', !!blurb && blurb.includes('files') && blurb.includes('read_file') && blurb.includes('mcp__<server>__<tool>'))
check('mcp prompt: excludes non-connected servers', !!blurb && !blurb.includes('down'))
check('mcp prompt: undefined when nothing connected', describeMcpForPrompt([{ id: 'd', status: 'disabled', tools: [] }]) === undefined && describeMcpForPrompt([]) === undefined)

// ---- Skills: frontmatter parse, prompt block, tool-output render (Phase 14) ----
check('skill: constants', SKILL_TOOL_NAME === 'skill' && SKILL_FILE_SAMPLE_LIMIT === 10 && SKILL_TOOL_DESCRIPTION.includes('skill'))

// parseSkillFrontmatter: happy path — scalar keys + body split.
const fmA = parseSkillFrontmatter('---\nname: pdf\ndescription: Fill PDF forms\n---\nDo the thing.\n')
check('skill fm: reads name + description', fmA.data.name === 'pdf' && fmA.data.description === 'Fill PDF forms')
...
check('skill fm: no frontmatter → empty data + body', Object.keys(fmNone.data).length === 0 && fmNone.body === 'just a body, no matter')
...
check('skill fm: skips list/nested/comment/block-scalar', fmList.data.name === 'x' && !('references' in fmList.data) && !('body' in fmList.data))
...
const skA: SkillInfo = { name: 'pdf', description: 'Fill & sign', location: '/s/pdf/SKILL.md', content: 'body', source: 'workspace' }
const skB: SkillInfo = { name: 'aws', location: '/g/aws/SKILL.md', content: 'body', source: 'global' }
const promptBlock = describeSkillsForPrompt([skB, skA])
check('skill prompt: wraps in <available_skills>', !!promptBlock && promptBlock.includes('<available_skills>') && promptBlock.includes('</available_skills>'))
check('skill prompt: sorted by name (pdf after aws)', !!promptBlock && promptBlock.indexOf('<name>aws</name>') < promptBlock.indexOf('<name>pdf</name>'))
check('skill prompt: lists name + location', !!promptBlock && promptBlock.includes('<name>pdf</name>') && promptBlock.includes('/s/pdf/SKILL.md'))
check('skill prompt: escapes XML in description', !!describeSkillsForPrompt([{ name: 'x', description: 'a & b <c>', location: '/x', content: '', source: 'global' }])?.includes('a &amp; b &lt;c&gt;'))
check('skill prompt: omits <description> when absent', !!promptBlock && promptBlock.includes('<name>aws</name>\n    <location>'))
...
const rtParsed = parseSkillFrontmatter(serializeSkillMarkdown('release-notes', 'Draft the release notes', '# Steps\nDo it.\n'))
check('skill serialize: round-trips name', rtParsed.data.name === 'release-notes')
check('skill serialize: round-trips description', rtParsed.data.description === 'Draft the release notes')
check('skill serialize: round-trips body', rtParsed.body.includes('# Steps') && rtParsed.body.includes('Do it.'))
...
check('skill serialize: colon in description survives', rtColon.data.description === 'Ratio 3:2 export')
...
check('skill serialize: special-lead description survives', rtQuoted.data.description === '#hashy value')
...
check('skill serialize: omits empty description', rtNoDesc.data.description === undefined && rtNoDesc.body.includes('Body only'))
...
const rendered = renderSkillContent({ name: 'pdf', content: '  # How\nSteps.  ' }, '/s/pdf', ['scripts/fill.py', 'reference/spec.md'])
check('skill render: wraps in <skill_content>', rendered.includes('<skill_content name="pdf">') && rendered.trimEnd().endsWith('</skill_content>'))
check('skill render: trims body + states base dir', rendered.includes('Steps.') && rendered.includes('Base directory for this skill: /s/pdf'))
check('skill render: lists sampled files', rendered.includes('<skill_files>') && rendered.includes('<file>scripts/fill.py</file>'))
...
check('skill src: owner/repo shorthand → github-repo', rsRepo.kind === 'github-repo' && rsRepo.owner === 'vercel-labs' && rsRepo.repo === 'agent-skills')
const rsRepoUrl = resolveSkillSource('https://github.com/vercel-labs/agent-skills')
check('skill src: github repo URL → github-repo', rsRepoUrl.kind === 'github-repo' && rsRepoUrl.repo === 'agent-skills')
const rsGit = resolveSkillSource('https://github.com/vercel-labs/agent-skills.git')
check('skill src: .git suffix stripped', rsGit.kind === 'github-repo' && rsGit.repo === 'agent-skills')
const rsTree = resolveSkillSource('https://github.com/vercel-labs/agent-skills/tree/main/skills/web-design-guidelines')
check(
  'skill src: /tree/<ref>/<path> → github-dir with ref+path',
  rsTree.kind === 'github-dir' && rsTree.ref === 'main' && rsTree.path === 'skills/web-design-guidelines'
)
...
check('skill src: owner/repo/sub/dir → github-dir (default branch)', rsShortPath.kind === 'github-dir' && rsShortPath.path === 'skills/hello' && rsShortPath.ref === undefined)
const rsScp = resolveSkillSource('git@github.com:o/r.git')
check('skill src: git@github SCP URL → github-repo', rsScp.kind === 'github-repo' && rsScp.owner === 'o' && rsScp.repo === 'r')
...
check('skill src: gitlab → unsupported (friendly)', rsGitlab.kind === 'unsupported' && /gitlab/i.test((rsGitlab as { reason: string }).reason))
...
check('skill sanitize: spaces → dashes', sanitizeSkillName('Web Design Guidelines') === 'web-design-guidelines')
check('skill sanitize: strips leading non-alnum', sanitizeSkillName('__weird--name') === 'weird--name')
...
async function main(): Promise<void> {
  ...
  check('mapWithConcurrency([]) is empty', (await mapWithConcurrency([], 4, async () => 1)).length === 0)
